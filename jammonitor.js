var JamMonitor = (function() {
    ...
    function initVpsConfig() {
        var savedUrl = localStorage.getItem('jammonitor_vps_url');
        var urlInput = document.getElementById('vps-url-input');
        var downloadSection = document.getElementById('vps-download-section');
        var statusBadge = document.getElementById('vps-status-badge');

        if (savedUrl && urlInput) {
            urlInput.value = savedUrl;
            if (downloadSection) downloadSection.style.display = 'block';
            checkVpsStatus(savedUrl);
        }
    }
    ...
    function saveVpsUrl() {
        ...
        var statusDiv = document.getElementById('vps-download-status');
        var downloadSection = document.getElementById('vps-download-section');

        if (!url) {
            localStorage.removeItem('jammonitor_vps_url');
            if (downloadSection) downloadSection.style.display = 'none';
            updateVpsBadge('Not Configured', '#95a5a6');
            if (statusDiv) statusDiv.innerHTML = '<span style="color:#95a5a6;">VPS URL cleared</span>';
            return;
        }

        // Validate URL format
        if (!url.match(/^https?:\/\/.+/)) {
            if (statusDiv) statusDiv.innerHTML = '<span style="color:#e74c3c;">Invalid URL format</span>';
            return;
        }

        localStorage.setItem('jammonitor_vps_url', url);
        if (downloadSection) downloadSection.style.display = 'block';
        if (statusDiv) statusDiv.innerHTML = '<span style="color:#27ae60;">URL saved! Checking connection...</span>';

        checkVpsStatus(url);
    }

    function checkVpsStatus(url) {
        var statusDiv = document.getElementById('vps-download-status');
        var sampleCountDiv = document.getElementById('vps-sample-count');

        fetch(url + '/status', { method: 'GET', mode: 'cors' })
            .then(function(r) {
                if (!r.ok) throw new Error('Status ' + r.status);
                return r.json();
            })
            .then(function(data) {
                updateVpsBadge('Connected', '#27ae60');
                if (statusDiv) statusDiv.innerHTML = '';
                if (sampleCountDiv && data.sample_count !== undefined) {
                    var oldest = data.oldest_timestamp ? new Date(data.oldest_timestamp * 1000).toLocaleString() : 'N/A';
                    sampleCountDiv.innerHTML = 'Samples stored: <strong>' + data.sample_count + '</strong> | Oldest: ' + oldest;
                }
            })
            .catch(function(e) {
                updateVpsBadge('Unreachable', '#e74c3c');
                if (statusDiv) statusDiv.innerHTML = '<span style="color:#f39c12;">VPS unreachable - will fall back to local diagnostics</span>';
                if (sampleCountDiv) sampleCountDiv.innerHTML = '';
            });
    }

    function updateVpsBadge(text, color) {
        var badge = document.getElementById('vps-status-badge');
        if (badge) {
    ...
    function downloadVpsBundle() {
        var vpsUrl = localStorage.getItem('jammonitor_vps_url');
        var hoursSelect = document.getElementById('vps-hours-select');
        var hours = hoursSelect ? hoursSelect.value : '24';
        var statusDiv = document.getElementById('vps-download-status');
        var btn = document.getElementById('vps-download-btn');

        if (!vpsUrl) {
            // No VPS configured - use local diagnostics
            exportDiag();
            return;
        }
        ...
        if (statusDiv) statusDiv.innerHTML = '<span style="color:#7f8c8d;">Fetching historical data from VPS...</span>';

        fetch(vpsUrl + '/bundle?hours=' + hours, { method: 'GET', mode: 'cors' })
            .then(function(r) {
            ...
            .then(function(blob) {
                ...
                a.href = url;
                a.download = 'jammonitor-history-' + hours + 'h-' + new Date().toISOString().slice(0,19).replace(/[T:]/g, '-') + '.json.gz';
                a.click();
                URL.revokeObjectURL(url);

                if (btn) {
                    btn.disabled = false;
                    btn.textContent = 'Download Historical Bundle';
                }
                if (statusDiv) statusDiv.innerHTML = '<span style="color:#27ae60;">Download complete!</span>';
                setTimeout(function() { if (statusDiv) statusDiv.innerHTML = ''; }, 3000);
            })
            .catch(function(e) {
                console.error('VPS download failed:', e);
                if (btn) {
                    btn.disabled = false;
                    btn.textContent = 'Download Historical Bundle';
                }
                if (statusDiv) statusDiv.innerHTML = '<span style="color:#f39c12;">VPS unreachable - falling back to local diagnostics...</span>';
                // Fall back to local diagnostics
                setTimeout(function() {
                    exportDiag();
                    if (statusDiv) statusDiv.innerHTML = '';
                }, 1000);
    ...
    // Initialize VPS config after DOM ready
    document.addEventListener('DOMContentLoaded', initVpsConfig);
    ...
    return {
        ...
        saveVpsUrl: saveVpsUrl,
        downloadVpsBundle: downloadVpsBundle
    };
})();
